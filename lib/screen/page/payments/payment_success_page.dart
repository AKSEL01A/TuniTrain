...
import 'package:tuni_train/screen/page/purchase/tickets/send_sheet.dart';
...
class _PaymentSuccessPageState extends State<PaymentSuccessPage>
  ...
  // ── Controllers ──────────────────────────────────────────────────────────
  late final TicketController ticketCtrl;
  ...
  void initState() {
    ...
    // Init TicketController with payment method from arguments
    ticketCtrl = Get.put(TicketController());
    ticketCtrl.paymentMethod = Get.arguments?['paymentMethod'] ?? 'wallet';
    ...
    _runSequence();
    ticketCtrl.saveTicket(); // 🔥 Save to Firestore automatically
  }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  _buildAnimated(offset: 30, child: _buildTitleBlock()),
                  const SizedBox(height: 32),
                  _buildAnimated(offset: 50, child: _buildTicketCard()),
                  const SizedBox(height: 20),
                  _buildAnimated(offset: 70, child: _buildInfoChips()),
                  const SizedBox(height: 32),
                  _buildAnimated(offset: 90, child: _buildActions()),
                ],
              ),
            ),
          ),
  ...
  Widget _buildAnimated({required Widget child, double offset = 30}) {
    return AnimatedBuilder(
      animation: _slideAnim,
      builder: (_, c) => Opacity(
        opacity: _fadeAnim.value.clamp(0.0, 1.0),
        child: Transform.translate(
          offset: Offset(0, offset * (1 - _slideAnim.value)),
          child: c,
        ),
      ),
      child: child,
  ...
  Widget _buildTicketCard() {
    return Obx(() {
      final code = ticketCtrl.savedTicket.value?.ticketCode ?? '—';
      return Container(
        decoration: BoxDecoration(
          color: AppColors.blue1,
        ...
        child: Column(
          children: [
            ...
            if (onTap != null) ...[
              ...
              Icon(Icons.touch_app_rounded, color: iconColor, size: 12),
            ],
          ],
        ),
      ),
  ...
  Widget _buildActions() {
    return Column(
      children: [
        ...
        const SizedBox(height: 12),
        GestureDetector(
          onTap: () => Get.offAllNamed('/searchtrain'),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              const Icon(
  ...
  void _openQrScreen() {
    Get.to(
      () => QrTicketScreen(ticketCtrl: ticketCtrl),
  ...
  void _showSendDialog() {
    ticketCtrl.resetSend();
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => SendSheet(ctrl: ticketCtrl),
    );
  }
}
...
class _Particle {
  late final double x, speed, size, rotation, rotSpeed, wobble;
